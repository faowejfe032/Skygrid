...
public class NetherChunkGen implements ChunkGen {
    ...
    public NetherChunkGen(FileConfiguration config, boolean onePoint17Plus) {
        ...
        for(String key : config.getConfigurationSection("nether").getKeys(false)) {
            ...
            if (Material.getMaterial(key) != null && Material.getMaterial(key).isBlock()) {
                ...
                probabilities.add(totalProbability += (config.getDouble("nether." + key) / 100));
        ...
        if(totalProbability < 1) {
            ...
            System.out.println(
                    "[Nether Generator] " + (1 - totalProbability) * 100 + "% of probability has not been allocated. "
            ...
            System.out.println(
                    "[Nether Generator] " + (totalProbability - 1) * 100 + "% of probability has been over-allocated. "
    ...
    private void handleCrimsonNylium(int x, int y, int z, ChunkData chunkData) {
